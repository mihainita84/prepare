...
def _move_crosses_barrier(barrier: np.ndarray, r: int, c: int, nr: int, nc: int) -> bool:
    """Check raster cells crossed by a longer propagation step.

    This prevents the extra 16-direction links from jumping across a drawn
    control line between the source and destination cells.
    """
    if barrier[r, c] or barrier[nr, nc]:
        return True
    dr, dc = nr - r, nc - c
    ...
    for k in range(1, steps):
        rf = r + dr * k / steps
        cf = c + dc * k / steps
        rr_candidates = {int(math.floor(rf)), int(math.ceil(rf))}
        cc_candidates = {int(math.floor(cf)), int(math.ceil(cf))}
        for rr in rr_candidates:
            for cc in cc_candidates:
                if 0 <= rr < barrier.shape[0] and 0 <= cc < barrier.shape[1] and barrier[rr, cc]:
                    return True
    return False
...
def simulate_spread(
    ...
    while pq:
        ...
        for dr, dc in neighbors:
            ...
            if _move_crosses_barrier(barrier, r, c, nr, nc):
